...
def test_un_solo_as():
    dados = [1, 3, 5, 2, 6]
    contador = ContadorPintas()
    assert contador.contar_pintas(dados, 3) == 2 #apostando a 3 debe sumar el 3 real + el As comodín


def test_ases_comodines_varios():
    dados = [1, 1, 4, 6, 2]
    contador = ContadorPintas()
    assert contador.contar_pintas(dados, 4) == 3  # apostando a 4 -> 1 (real) + 2 (ases) = 3
    assert contador.contar_pintas(dados, 2) == 3  # apostando a 2 -> 1 (real) + 2 (ases) = 3
...
def test_as_no_comodin_en_ronda_de_un_dado():
    ...
    dados = [1]
    contador = ContadorPintas()
    assert contador.contar_pintas(dados, 5, ases_comodines=False) == 0  # si apuesto a 5 -> 0
    assert contador.contar_pintas(dados, 1, ases_comodines=False) == 1  # si apuesto a 1 -> 1


def test_todos_ases():
    dados = [1, 1, 1, 1, 1]
    contador = ContadorPintas()
    assert contador.contar_pintas(dados, 6) == 5  # apostando a 6 -> todos los Ases cuentan como comodín -> 5
    assert contador.contar_pintas(dados, 1) == 5   # apostando a 1 -> también 5 (As cuenta como sí mismo y comodín)
...
def test_contar_pintas_numero_no_en_dados_con_ases():
    ...
    contador = ContadorPintas()
    apuesta = 5
    resultado = contador.contar_pintas(dados, apuesta, ases_comodines=True)
